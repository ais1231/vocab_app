...
| **原创按键音效** | 三个学习判断键使用原创音频片段，其余操作使用轻量实时合成音，可在设置中关闭 |
...
├── assets/               # 字体与原创 UI 音效资源
├── scripts/              # 可重复运行的资源生成脚本
├── tests/                # 功能、界面与桌面窗口回归测试
...
`build/`、`dist/` 与 `__pycache__/` 均为本地生成目录，不纳入版本控制；历史安装包可从 Git 记录或 GitHub Releases 获取。
